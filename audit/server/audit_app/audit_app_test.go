...
func Test_list(t *testing.T) {
	list, err := GetAuditAppList(&ReqSearch{
...
//授权
func Test_audit_app(t *testing.T) {
	list, err := AuthApp(&server.AuthReq{
		User:  &request.UserReq{AdminUserId: 1},
		Id:    26,
...
func Test_log_list(t *testing.T) {
	list, err := GetHostLog(&AppLogReq{
		UserId:   &request.UserReq{AdminUserId: 1},
		AuditId:  []string{"host-xxxx"},
		TimeType: "30day",
		Page:     1, Size: 10,
		Message: "183.136.225.35",
		Export:  true,
	})
	fmt.Println(list)
	fmt.Println(err)
