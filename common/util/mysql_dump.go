...
func (d *Dumper) Dump(tableName []string) (string, error) {
	...
	// Get tables
	tables, err := getTables(d.db, tableName)
	if err != nil {
		return p, err
	}
...
func Reduction(db *sql.DB, sql *string) (err error) {
	tx, err := db.Begin()
	if err != nil {
		return
	}
...
func getTables(db *sql.DB, tableName []string) ([]string, error) {
	...
	for rows.Next() {
		...
		for key, value := range data {
			if value != nil && value.Valid {
				value.String = strings.ReplaceAll(value.String, `\r`, `\\r`)
				value.String = strings.ReplaceAll(value.String, `\n`, `\\n`)
				value.String = strings.ReplaceAll(value.String, `"`, `\"`)
