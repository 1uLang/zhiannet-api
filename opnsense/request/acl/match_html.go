...
func ListMatch(Interface string, data io.Reader) (list []*AclListResp, err error) {
	...
	doc.Find("#iform table tbody tr.rule ").Each(func(i int, s *goquery.Selection) {
		...
		//	info.Disabled = true
		//}
		text, _ := s.Find("td").Eq(1).Find("i").Eq(0).Attr("title")
		if text == "禁用规则" {
			info.Disabled = true
		}
		//方向
...
func InfoMatch(data io.Reader) (info *AclInfoResp, err error) {
	...
	doc.Find("#iform .opnsense_standard_table_form").Each(func(i int, s *goquery.Selection) {
		...
		//源端口 开始
		srcBeginPort := make([]SelectedParams, 0)
		s.Find("tr").Find("select[name='srcbeginport'] option").Each(func(x int, sel *goquery.Selection) {
			dataOther := false
			if _, ok := sel.Attr("data-other"); ok {
				dataOther = true
			}
		...
		//目标端口 开始
		dstBeginPort := make([]SelectedParams, 0)
		s.Find("tr").Find("select[name='dstbeginport'] option").Each(func(x int, sel *goquery.Selection) {
			dataOther := false
			if _, ok := sel.Attr("data-other"); ok {
				dataOther = true
			}
